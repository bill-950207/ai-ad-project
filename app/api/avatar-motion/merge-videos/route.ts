 ...
 * 아바타 모션 영상 합치기 API
 *
 * POST: 여러 씬 영상 URL을 받아서 하나의 비디오로 합칩니다.
 * - FFmpeg를 사용하여 비디오 연결
 * - 결과를 R2에 업로드
 */
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(`[Avatar Motion] Merging ${videoUrls.length} scene videos...`)
    ...
    const key = `avatar-motion/merged/${draftId || user.id}_merged_${timestamp}.mp4`
    ...
    console.log(`[Avatar Motion] Merged video uploaded: ${mergedVideoUrl}`)

    return NextResponse.json({
      mergedVideoUrl,
      message: `Successfully merged ${videoUrls.length} scene videos`,
    })
  } catch (error) {
    console.error('[Avatar Motion] 비디오 합치기 오류:', error)
