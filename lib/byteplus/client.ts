 ...
 *
 * 현재: Seedance 1.5 Pro로 API 패턴 검증
 * 2/24 이후: SEEDANCE_MODEL_ID 환경변수만 변경하여 Seedance 2.0 전환
 */
...
export async function createVideoTask(
  ...
  // Seedance 2.0 추가 파라미터 (2/24 출시 후 확정)
  // 현재는 기본값 사용, 출시 후 API 스펙에 맞게 조정
...
export async function cancelVideoTask(taskId: string): Promise<void> {
  ...
  if (!response.ok) {
    const errorText = await response.text()
    throw new Error(`BytePlus 태스크 취소 오류 (${response.status}): ${errorText}`)
  }
}
