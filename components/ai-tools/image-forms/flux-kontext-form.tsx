...
import { FLUX_KONTEXT_CREDIT_COST } from '@/lib/credits/constants'
import { useLanguage } from '@/contexts/language-context'

const ASPECT_RATIOS = ['1:1', '4:3', '3:4', '16:9', '9:16'] as const

interface FluxKontextFormProps {
  onSubmit: (data: {
    model: 'flux-kontext'
    prompt: string
    imageUrl?: string
...
export default function FluxKontextForm({ onSubmit, isGenerating }: FluxKontextFormProps) {
  const { t } = useLanguage()
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      >
        ...
        {isGenerating
          ...
          : `${aiToolsT.generate || '생성하기'} (${FLUX_KONTEXT_CREDIT_COST} ${aiToolsT.credits || '크레딧'})`
        }
      </button>
    </form>
  )
