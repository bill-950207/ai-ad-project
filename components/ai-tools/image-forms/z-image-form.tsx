...
import { Z_IMAGE_TOOL_CREDIT_COST } from '@/lib/credits/constants'
import { useLanguage } from '@/contexts/language-context'

const ASPECT_RATIOS = ['1:1', '4:3', '3:4', '16:9', '9:16'] as const

interface ZImageFormData {
  prompt: string
  aspectRatio: typeof ASPECT_RATIOS[number]
}

interface ZImageFormProps {
  onSubmit: (data: ZImageFormData & { model: 'z-image' }) => void
  isGenerating: boolean
}

export default function ZImageForm({ onSubmit, isGenerating }: ZImageFormProps) {
  const { t } = useLanguage()
    ...
    <form onSubmit={handleSubmit} className="space-y-5">
      ...
      >
        ...
        {isGenerating
          ...
          : `${aiToolsT.generate || '생성하기'} (${Z_IMAGE_TOOL_CREDIT_COST} ${aiToolsT.credits || '크레딧'})`
        }
      </button>
    </form>
  )
